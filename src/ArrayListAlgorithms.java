...
public class ArrayListAlgorithms {
    ...
    public static void replaceWithCaps(ArrayList<String> wordList)
    {
        for (int i =0;i<wordList.size();i++){
            if (wordList.get(i).substring(wordList.get(i).length()-1).equals("s")) {
               String r= wordList.get(i).toUpperCase(Locale.ROOT);
               wordList.add(i,r);
               wordList.remove(i+1);
            }
        }
    ...
    {
        for (int i = 0; i<wordList.size();i++){
            wordList.add(i,wordList.get(i).toUpperCase(Locale.ROOT));
            i++;
        }
    }
    ...
    {
        int in = wordList.size();
        for (int i = 0; i<in;i++){
        wordList.add(wordList.get(i).toUpperCase(Locale.ROOT));
    } }
